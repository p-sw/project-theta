...
export class AuthController {
   ...
   */
  @TypedRoute.Get(':provider/submit')
  ...
  async submitOAuthSession(
    ...
    let userId = await this.authService.getUserIdByOAuth(
      provider,
      oauthUser.data,
      _logger,
    );
    ...
    if (!userId) {
      ...
      userId = await this.authService.createUserByOAuth(
        provider,
        oauthUser.data.id,
        accessTokenTry.data,
        _logger,
    ...
    const sessionId = await this.authService.createSession(userId, _logger);
